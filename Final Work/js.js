...
let users = [
 ...
 const data={users};
  ...
  function showusers(){
    ...
    ul.innerHTML=liText;
    console.log(users);
    const newList = data.users.forEach((elem, ind) =>{
  ...
  function deleteAll(){
    ...
    if(ans){
      data.users.length=0;console.log(users);
...
function editUser(ind){
  ...
  function serchSome(){
    document.getElementById('serchIn').addEventListener('input' ,filterList );


    function filterList(){
      const searchInput=document.getElementById('serchIn'); 
      const sInput=searchInput.value.toLowerCase();
      data.users.forEach((item)=>{
        let name=item.firstname.toLowerCase();
        if(name.startsWith(sInput)){
